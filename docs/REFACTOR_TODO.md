...
- [ ] ADR-0012 says tasks record "the planner/distributor independently of assignees" — there's no `assigneeId` anywhere, only `createdById`. Either add the field or soften the doc to match what's actually modeled.
...
## Authorization — ADR-0019 policy module (decided 2026-07-02, hand-implement)

Design in `docs/adr/0019-centralized-authorization-policy.md`. Current state: authorization lives in 19 scattered ownership checks across services — `task.service.ts` (8× `project.userId`), `project.service.ts` (6× `project.userId`), `timeEntry.service.ts` (2× `task.createdById`, 2× `entry.userId`), `activeTask.service.ts` (1× `task.createdById`). Actions do authentication only (21/21 call `auth()`); server components check sessions themselves.
...
- [ ] **Decide the ownership question the migration forces:** is time-entry authority `task.createdById` or `task.project.userId`? Identical solo, divergent under ADR-0012 team mode. Pick one, encode it in `can()`, note it in ADR-0019.
...
- [ ] **Update ADR-0017's consequences note** once migrated: missing checks in services are no longer "by design"; a service touching an owned resource without `assertCan` is a review finding.
- [ ] Optional, complementary (not part of ADR-0019): `withAuth` action wrapper minting a branded `AuthenticatedUserId` — kills the ~20-line per-action boilerplate and makes "forgot `auth()`" impossible. Decide separately.
