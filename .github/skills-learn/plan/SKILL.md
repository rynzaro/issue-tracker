...
Read `docs/AGENT.md` (iteration status, architecture rules) and `docs/ROADMAP.md` (what's done/pending).
Deliberate on scope using Sequential Thinking MCP before proceeding.

### Step 2 — Agent Plans First (Hidden)

Generate your own decomposition BEFORE seeing the user's. This prevents anchoring to their framing. Hold it internally.
...
- **What they missed** — explain what thinking would have caught it (e.g., "the architecture rule in AGENT.md §2 means this needs a server action layer between UI and service")
...
- **Architecture & Patterns**: Did they route work through the right layers (service → action → component)?
...
- **Top-down (senior)**: "The mutation needs a server action (auth + validate), which calls a new service function, which needs a Zod schema — so schema first, then service, then action, then UI."
- **Bottom-up (mid-level)**: "Create the form, add validation, connect to API."
- **Feature-list (junior)**: "Add the button, make it save, show a success message."
