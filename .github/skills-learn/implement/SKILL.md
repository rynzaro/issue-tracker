...
Read `docs/AGENT.md`. Check iteration status for which rules apply.
Read relevant service/action files to understand existing patterns.
...
2. **Hints**: Give 2–3 hints that point to the RIGHT FILE or PATTERN to reference, not the solution itself. E.g., "Look at how `deleteTask` in `task.service.ts` handles the BFS cascade — you'll need the same pattern here."
...
- "That's correct — but what's the specific failure mode? What data would be inconsistent?"
- "When would the alternative approach actually be better?"
